# Smart Grocery Agent

AI Agent와 MCP(Model Context Protocol)를 활용하여 사용자 상황에 맞는 장보기 목록을 생성하는 스마트 장보기 도우미 서비스입니다.

## 프로젝트 소개

Smart Grocery Agent는 가족 구성, 알레르기, 장보기 목적, 예산 등의 정보를 바탕으로 개인화된 식단과 장보기 목록을 생성하는 멀티 에이전트 기반 서비스입니다.

사용자는 웹 UI를 통해 장보기 조건을 입력할 수 있으며, 여러 Agent가 협력하여 메뉴 추천, 재료 추출, 가격 비교 및 예산 최적화를 수행합니다.

## 문제 정의

일반적인 장보기 과정에서는 다음과 같은 문제가 발생합니다.

* 무엇을 구매해야 할지 결정하기 어렵다.
* 가족 구성원 수에 따라 필요한 식재료 양이 달라진다.
* 알레르기 식품이 포함될 수 있다.
* 예산을 초과하는 구매가 발생한다.
* 이미 보유 중인 재료를 다시 구매하는 경우가 있다.

본 프로젝트는 이러한 문제를 AI Agent와 MCP 도구를 활용하여 해결하는 것을 목표로 한다.

## 서비스 대상 사용자

* 주간 장보기를 하는 가정
* 다이어트 식단을 관리하는 사용자
* 알레르기 및 식이 제한이 있는 사용자
* 예산 기반 장보기를 원하는 사용자
* 캠핑 및 파티를 준비하는 사용자

## 핵심 기능

### 1. 사용자 상황 분석

입력 정보

* 가족 수
* 알레르기 정보
* 장보기 목적
* 예산

분석 결과를 Agent가 활용하여 맞춤형 장보기 계획을 생성한다.

### 2. 메뉴 추천

외부 레시피 데이터를 활용하여 메뉴 후보를 수집한다.

다음 요소를 고려하여 메뉴를 추천한다.

* 장보기 목적
* 알레르기
* 가족 수
* 대중성 필터

예시

* Chicken Pasta
* Grilled Chicken
* Vegetable Soup
* Beef Rice Bowl

### 3. 알레르기 필터링

사용자가 입력한 알레르기 정보를 기반으로 위험 재료를 자동 제거한다.

예시

* 계란
* 우유
* 닭고기
* 땅콩
* 새우

### 4. 가격 비교 및 예산 최적화

Price MCP가 식재료 가격을 추정하고 매장별 가격을 비교한다.

지원 매장

* Coupang
* Emart
* Homeplus

예산을 초과하는 경우 우선순위가 낮은 품목을 제외하여 최종 장보기 목록을 생성한다.

### 5. 가족 규모 기반 수량 조정

가족 수에 따라 구매 수량을 자동 조정한다.

예시

* 1인 가구 → 1배
* 3~4인 → 1.5배
* 5~6인 → 2배
* 7인 이상 → 2.5배

### 6. 외부 레시피 데이터 필터링

초기에는 TheMealDB에서 가져온 메뉴를 그대로 추천하였다. 그러나 테스트 과정에서 한국 사용자에게 익숙하지 않은 메뉴나 장보기 목록으로 적합하지 않은 재료가 포함되는 문제가 있었다.

이를 해결하기 위해 Menu Filter MCP를 추가하였다.

Menu Filter MCP는 메뉴명과 재료 목록을 분석하여 다음 기준으로 메뉴를 평가한다.

* 대중적인 메뉴인지 여부
* 장보기 목적과 맞는지 여부
* 생소한 재료가 포함되어 있는지 여부
* 디저트 메뉴가 적절한 목적에서만 사용되는지 여부

이를 통해 TheMealDB라는 외부 데이터 소스는 유지하면서도 실제 사용자에게 더 자연스러운 장보기 결과를 제공할 수 있도록 개선하였다.


## 시스템 구조

Web UI

↓

FastAPI Backend

↓

Master Agent

├── Menu Planner Agent

├── Inventory Agent

└── Price Optimizer Agent

↓

MCP Servers

├── Recipe MCP

└── Price MCP

## 실행 방법

### Backend

```bash
pip install -r requirements.txt
uvicorn app:app --reload
```

### Frontend

브라우저 접속

```text
http://127.0.0.1:8000
```

## 사용 기술

* Python
* FastAPI
* MCP
* HTML/CSS/JavaScript
* TheMealDB API

## 향후 개선 방향

* 실제 커머스 API 연동
* 실시간 가격 비교 기능
* LLM 기반 메뉴 추천
* OCR 영수증 분석
* 냉장고 재고 관리 MCP 연동

## 프로젝트 특징

본 프로젝트는 Agent와 MCP를 분리하여 구현하였다.

Agent는 의사결정을 담당하며 MCP는 외부 데이터 접근 및 가격 비교 기능을 담당한다.

이를 통해 향후 실제 커머스 API나 외부 데이터 소스로 쉽게 확장할 수 있다.
